...
// --- Sound and Visual Effects Helpers (place outside the component for 1:1 mapping with example) ---
...
const drawCracksAnimated = (crackCtx: CanvasRenderingContext2D, crackCanvas: HTMLCanvasElement, cx: number, cy: number, duration = 900, animationFrameIdRef: React.MutableRefObject<number | undefined>) => {
    ...
    function drawPathPartial(points: any[], prog: number, width: number){
    ...
    function frame(now: number){
        ...
        if(globalP < 1) animationFrameIdRef.current = requestAnimationFrame(frame);
    }
...
const flashAt = (stage: HTMLDivElement, x: number, y: number) => {
    ...
    for (let r=0;r<rows;r++){
        for (let c=0;c<cols;c++){
            ...
            [triA,triB].forEach(poly => {
                ...
                sCanvas.style.position = 'absolute'; sCanvas.style.left = (minX) + 'px'; sCanvas.style.top = (minY) + 'px'; sCanvas.style.willChange = 'transform, opacity';
                const sCtx = sCanvas.getContext('2d'); if(!sCtx) return;
...
const animateShards = (shards: any[], shardsContainer: HTMLDivElement, fade: HTMLDivElement, onComplete: () => void, animationFrameIdRef: React.MutableRefObject<number | undefined>) => {
    shards.forEach(s => { shardsContainer.appendChild(s.el);
        const impulse = 3 + Math.random()*5; s.vx = s.nx * (impulse * (0.6 + Math.random()*1.4)) + (Math.random()-0.5)*2;
...
const startShatter = async (
    content: HTMLElement, stage: HTMLDivElement, crackCanvas: HTMLCanvasElement, crackCtx: CanvasRenderingContext2D,
    shardsContainer: HTMLDivElement, fade: HTMLDivElement, onComplete: () => void, animationFrameIdRef: React.MutableRefObject<number | undefined>
) => {
    const audio = getAudioContext();
    if (audio && audio.state === 'suspended') await audio.resume();
    
    const impactX = window.innerWidth/2; const impactY = window.innerHeight/2;
    content.classList.add('pulse'); setTimeout(()=> content.classList.remove('pulse'), 250);
    playImpact();
    drawCracksAnimated(crackCtx, crackCanvas, impactX, impactY, 1000, animationFrameIdRef);
    await wait(900);
    ...
    try {
        ...
        flashAt(stage, impactX, impactY);
        playShatter();
        const shards = makeShardsFromImage(canvasSnapshot, impactX, impactY, 20, 12);
        content.style.visibility = 'hidden';
        crackCtx.clearRect(0,0,crackCanvas.width, crackCanvas.height);
        animateShards(shards, shardsContainer, fade, onComplete, animationFrameIdRef);
    } catch (error) {
        console.error("html2canvas failed, falling back gracefully.", error);
...
const FinalShatterEffect: React.FC<{ onComplete: () => void }> = ({ onComplete }) => {
    ...
    useEffect(() => {
        ...
        if (!content || !stage || !crackCanvas || !crackCtx || !shardsContainer || !fade) return;
        ...
        startShatter(content, stage, crackCanvas, crackCtx, shardsContainer, fade, onComplete, animationFrameId);
        ...
        return () => {
            ...
            if (shardsContainer) {
                shardsContainer.innerHTML = ''; // Clean up generated canvases
            }
        };
    }, [onComplete]);

    return (
        <div id="shatter-stage" ref={stageRef}>
